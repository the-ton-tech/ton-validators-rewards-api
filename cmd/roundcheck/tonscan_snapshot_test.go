...
func TestTonscanElectionSnapshots(t *testing.T) {
	...
	for _, entry := range entries {
		...
		t.Run(name, func(t *testing.T) {
			...
			// Every JS validator must have been an election participant, so the JS
			// elected set cannot exceed the total tonscan participant count.
			if d.TotalParticipants > 0 && len(jsIn.Validators) > d.TotalParticipants {
				t.Errorf("tonscan vs JS (%s): JS validator count %d > tonscan total_participants %d",
					jsPath, len(jsIn.Validators), d.TotalParticipants)
			}
		})
	}
